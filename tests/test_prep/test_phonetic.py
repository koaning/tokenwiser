...
@pytest.mark.parametrize("x_in,x_out", [("haleluja", "HLLJ"), ("hello there world", "HL 0R WRLT")])
def test_metaphone(x_in, x_out):
    assert PhoneticPrep(kind="metaphone").encode_single(x_in) == x_out


@pytest.mark.parametrize("x_in,x_out", [("haleluja", "HALALAJ"), ("hello there world", "HAL TAR WARLD")])
